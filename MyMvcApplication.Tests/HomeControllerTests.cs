...
namespace MyMvcApplication.Tests
{
    ...
    public class HomeControllerTests
    {
        ...
        private class MockAnimalSoundProvider : IAnimalSoundProvider
        {
        ...
        [Test]
        public void MockedAnimalSounds()
        {
            // This will execute after the context, application, controller have been constructed, but before the action
            // executes. In other words, you can mock controller's dependencies, for example if using Unity or other IoC.
            appHost.Start(bs =>
                {
                    var mockedAnimalSoundProvider = new MockAnimalSoundProvider();

                    FilterProviders.Providers.Add(new MockInjectionFilterProvider(context =>
                        {
                            var controller = (HomeController)context.Controller;
                            controller.AnimalSoundProvider = mockedAnimalSoundProvider;

                            return false;
                        }));

                    var requestResult = bs.Get("/Home/MakeAnimalSound/dog");

                    Assert.IsTrue(requestResult.ResponseText.Contains("The dog goes Mock! Mock!"));
                });
        }
